import torch
from s2pg.algorithms.imitation_learning.bptt_pI.iq_sac_bptt_pI import IQ_SAC_BPTT_pI
import torch.nn.functional as F

from mushroom_rl.utils.torch import to_float_tensor


class LSIQ_BPTT_pI(IQ_SAC_BPTT_pI):
    """
    ...
    def __init__(self, Q_max=1.0, Q_min=-1.0, loss_mode_exp="fix", Q_exp_loss=None,
        ...
        # call parent
        super(LSIQ_BPTT_pI, self).__init__(**kwargs)
    ...
    def _lossQ(self, obs, act, next_obs, absorbing, prev_action, action_seq, lengths, is_expert):
        if self._lossQ_type == "sqil_like":
            return self._lossQ_sqil_like(obs, act, next_obs, absorbing, prev_action, action_seq, lengths, is_expert)
        elif self._lossQ_type == "iq_like":
            return self._lossQ_iq_like(obs, act, next_obs, absorbing, prev_action, action_seq, lengths, is_expert)
        else:
            raise ValueError("Unsupported lossQ type %s" % self._lossQ_type)

    def _lossQ_iq_like(self, obs, act, next_obs, absorbing, prev_action, action_seq, lengths, is_expert):
        ...
        if self._plcy_loss_mode == "value":
            V = self.getV(next_obs, last_next_obs, prev_action, lengths)
            value = (V - y)
    ...
    def _lossQ_sqil_like(self, obs, act, next_obs, absorbing, prev_action, action_seq, lengths, is_expert):
        ...
        # policy part
        if self._plcy_loss_mode == "value":
            value = self.getV(obs, last_obs, prev_action, lengths)
            target = y
            r_min = torch.concat([r_min, torch.ones_like(r_min) * (-(1 / self._reg_mult))])
        elif self._plcy_loss_mode == "value_plcy":
            value = self.getV(obs[~is_expert], last_obs[~is_expert], prev_action[~is_expert], lengths[~is_expert])
            target = y[~is_expert]
        elif self._plcy_loss_mode == "q_old_policy":
