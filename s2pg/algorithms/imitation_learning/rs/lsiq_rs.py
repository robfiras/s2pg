import torch
from s2pg.algorithms.imitation_learning.rs.iq_sac_rs import IQ_SAC_RS
import torch.nn.functional as F

from mushroom_rl.utils.torch import to_float_tensor


class LSIQ_RS(IQ_SAC_RS):
    """
    Implementation of Least-Squares Inverse Q-Learning.
    ...
    Firas Al-Hafez, Davide Tateo, Oleg Arenz, Guoping Zhao, Jan Peters (2023)

    """
    def __init__(self, Q_max=1.0, Q_min =-1.0, loss_mode_exp="fix", Q_exp_loss=None,
                 treat_absorbing_states=False, target_clipping=True,
                 lossQ_type="iq_like", **kwargs):

        # call parent
        super(LSIQ_RS, self).__init__(**kwargs)
    ...
    def _lossQ(self, obs, act, next_obs, absorbing, prev_action, is_expert):
        if self._lossQ_type == "sqil_like":
            return self._lossQ_sqil_like(obs, act, next_obs, absorbing, prev_action, is_expert)
        elif self._lossQ_type == "iq_like":
            return self._lossQ_iq_like(obs, act, next_obs, absorbing, prev_action, is_expert)
        else:
            raise ValueError("Unsupported lossQ type %s" % self._lossQ_type)
        
    def _lossQ_iq_like(self, obs, act, next_obs, absorbing, prev_action, is_expert):
        ...
        if self._gradient_pen_coeff > 0.0:
            act = torch.autograd.Variable(act, requires_grad=True)
            next_hidden_state = torch.autograd.Variable(next_hidden_state, requires_grad=True)
        current_Q = self._critic_approximator(obs[:, self._critic_state_mask], act, next_hidden_state, prev_action,
                                              output_tensor=True)
        ...
        # gradient penalty
        if self._gradient_pen_coeff > 0.0:
            gradient_penalty = self.gradient_penalty(current_Q, act, next_hidden_state, is_expert)
        else:
            gradient_penalty = 0.0

        loss_Q = loss_term1 + loss_term2 + chi2_loss + gradient_penalty
    ...
    def _lossQ_sqil_like(self, obs, act, next_obs, absorbing, prev_action, is_expert):
        ...
        # policy part
        if self._plcy_loss_mode == "value":
            value = self.getV(obs, prev_action)
            target = y
        ...
        loss_Q = loss_term1 + loss_term2 + gradient_penalty
        self.update_Q_parameters(loss_Q)
