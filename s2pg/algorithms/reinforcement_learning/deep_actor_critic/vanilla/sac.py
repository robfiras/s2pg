...
class SACPolicy(Policy):
    ...
    """
    def __init__(self, approximator, min_a, max_a,
                 log_std_min, log_std_max, state_mask=None):
        """
        Constructor.

        Args:
            approximator (Regressor): a regressor computing mean and variance in given a
                state;
    ...
    def get_mu_log_sigma(self, state):
        ...
        if len(mu_log_sigma.size()) == 1:
            mu = mu_log_sigma[0: self._action_dim]
            log_sigma = mu_log_sigma[self._action_dim:2*self._action_dim]
...
class SAC(DeepAC):
    ...
    def __init__(self, mdp_info, actor_params,
                 ...
                 lr_alpha, init_alpha, policy_state_mask=None, log_std_min=-20,
                 log_std_max=2, target_entropy=None,
                 critic_fit_params=None, logging_iter=100, sw=None):
        """
        Constructor.
        ...
        actor_approximator = Regressor(TorchApproximator, **actor_params)
        ...
        super().__init__(mdp_info, policy, actor_optimizer, actor_approximator.model.network.parameters())
    ...
    def fit(self, dataset, **info):
        ...
        if self._replay_memory.initialized:
            ...
            if self._replay_memory.size > self._warmup_transitions():
                action_new, log_prob = self.policy.compute_action_and_log_prob_t(state[:, self._policy_state_mask])
            ...
            if self._iter % self._logging_iter == 0 and self._replay_memory.size > self._warmup_transitions():
                self.sw_add_scalar("Actor/Entropy", -torch.mean(log_prob), self._iter)
                self.sw_add_scalar("Actor/Mean Action", torch.mean(action_new), self._iter)
                self.sw_add_scalar("Actor/Variance Action", torch.var(action_new), self._iter)
                self.sw_add_scalar("Actor/Alpha", self._alpha, self._iter)
                self.sw_add_scalar("Critic/Target Q-value", np.mean(q), self._iter)
    ...
    def _next_q(self, next_state, absorbing):
        ...
        """
        a, log_prob_next = self.policy.compute_action_and_log_prob(next_state[:, self._policy_state_mask])
    ...
    def sw_add_scalar(self, tag, data, iter):
        if self._sw:
            self._sw.add_scalar(tag, data, iter)
