...
class DDPG(DeepAC):
    ...
    def __init__(self, mdp_info, policy_class, policy_params,
                 ...
                 initial_replay_size, max_replay_size, tau, policy_state_mask=None, policy_delay=1, sw=None,
                 logging_iter=10, critic_fit_params=None, actor_predict_params=None, critic_predict_params=None):
        """
        Constructor.
        ...
        if sw:
            self._sw = sw
            setattr(self._sw, '__deepcopy__',
                    lambda self: None)  # dont need to be copyable, causes pickle error otherwise
        else:
            self._sw = None
    ...
    def fit(self, dataset, **info):
        ...
        if self._replay_memory.initialized:
            ...
            if self._fit_count % self._logging_iter == 0 and self._sw:
                grads_pi = []
                for param in self._actor_approximator.model.network.parameters():
                    grads_pi.append(param.grad.view(-1))
                grads_pi = torch.cat(grads_pi)
                norm_pi = grads_pi.norm(dim=0, p=2)
                self._sw.add_scalar('Gradients/Norm2 Gradient Q wrt. Pi-parameters', norm_pi,
                                    self._fit_count)
            ...
            if self._fit_count % self._logging_iter == 0 and self._sw:
                ...
                self._sw.add_scalar("Mean Q", mean_q, self._fit_count)
                self._sw.add_scalar("Action/Mean", mean_action, self._fit_count)
                self._sw.add_scalar("Action/Var", var_action, self._fit_count)
    ...
    def _next_q(self, next_state, absorbing):
        ...
        """
        a = self._target_actor_approximator.predict(next_state[:, self._policy_state_mask], **self._actor_predict_params)
