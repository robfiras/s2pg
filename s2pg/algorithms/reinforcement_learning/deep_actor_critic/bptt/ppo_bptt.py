...
class PPO_BPTT(Agent):
    ...
    def fit(self, dataset, **info):
        ...
        v_target, np_adv = self.compute_gae(self._V, x_seq, xn_seq, prev_u_seq, u_seq, lengths,
                                            r, absorbing, last, self.mdp_info.gamma, self._lambda())
        np_adv = (np_adv - np.mean(np_adv)) / (np.std(np_adv) + 1e-8)
        adv = to_float_tensor(np_adv, self.policy.use_cuda)

        old_pol_dist = self.policy.distribution_t(obs, prev_act_seq, lengths)
        old_log_p = old_pol_dist.log_prob(act)[:, None].detach()

        self._V.fit(x_seq, prev_u_seq, lengths, v_target, **self._critic_fit_params)

        self._update_policy(obs, act, prev_act_seq, lengths, adv, old_log_p)

        # Print fit information
        self._log_info(dataset, x_seq, prev_u_seq, lengths, v_target, old_pol_dist)
        self._iter += 1
        ...
        for epoch in range(self._n_epochs_policy()):
            for obs_i, act_i, prev_act_seq_i, length_i, adv_i, old_log_p_i in minibatch_generator(
                ...
                prob_ratio = torch.exp(
                    self.policy.log_prob_t(obs_i, act_i, prev_act_seq_i, length_i) - old_log_p_i
                )
    ...
    def _log_info(self, dataset, x, prev_u, lengths, v_target, old_pol_dist):
        if self._sw:
            torch_v_targets = torch.tensor(v_target, dtype=torch.float)
            v_pred = torch.tensor(self._V(x, prev_u, lengths), dtype=torch.float)
            v_err = F.mse_loss(v_pred, torch_v_targets)

            logging_ent = self.policy.entropy(x)
            new_pol_dist = self.policy.distribution(x, prev_u, lengths)
    ...
    @staticmethod
    def compute_gae(V, s, ss, prev_u, u, lengths, r, absorbing, last, gamma, lam):
        """
        ...
        """
        v = V(s, prev_u, lengths)
        v_next = V(ss, u, lengths)
