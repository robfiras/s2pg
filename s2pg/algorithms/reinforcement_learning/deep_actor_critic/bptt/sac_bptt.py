...
class SAC_BPTT(DeepAC):
    ...
    def __init__(self, mdp_info, actor_params,
                 ...
                 lr_alpha, hidden_state_dim, action_dim,  init_alpha=0.1,
                 logging_iter=1000, log_std_min=-20, log_std_max=2, target_entropy=None,
        ...
        if 'n_models' in critic_params.keys():
            assert critic_params['n_models'] == 1
        else:
        ...
        policy = SACBPTTPolicy(actor_approximator,
    ...
    def fit(self, dataset, **info):
        ...
        if self._replay_memory.initialized:
            ...
            if self._replay_memory.size > self._warmup_transitions():
                action_new, log_prob = self.policy.compute_action_hidden_and_log_prob_t(state, prev_action, lengths)
                loss = self._loss(state, prev_action, action_new, log_prob, lengths)
                self._optimize_actor_parameters(loss)
                self._update_alpha(log_prob.detach())

            q_next = self._next_q(next_state, action_seq, absorbing, lengths)
            q = reward + self.mdp_info.gamma * q_next
            q = np.stack([q, q], axis=1)
            ...
            self._critic_approximator.fit(state, action, prev_action, lengths, q,
    ...
    def _loss(self, state, prev_action, action, log_prob, lengths):
        q = self._critic_approximator(state, action, prev_action, lengths,
    ...
    def _next_q(self, next_state, action, absorbing, lengths):
        """
        Args:
        ...
        """
        next_action, log_prob_next = self.policy.compute_action_hidden_and_log_prob(next_state, action, lengths)

        q = self._target_critic_approximator.predict(next_state, next_action, action, lengths)

        q = np.min(q, axis=1) - self._alpha_np * log_prob_next
