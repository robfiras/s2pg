...
from s2pg.algorithms import LSIQ_BPTT_pI
from s2pg.networks.vanilla_networks import FullyConnectedNetwork
from s2pg.networks.recurrent_networks import SACBPTTActorNetwork
...
def experiment(env: str = "HumanoidTorque",
    ...
    actor_params = dict(network=SACBPTTActorNetwork,
    ...
    # create IQfO agent
    agent = LSIQ_BPTT_pI(mdp_info=mdp.info, batch_size=batch_size, initial_replay_size=initial_replay_size,
                         ...
                         actor_optimizer=actor_optimizer, critic_params=critic_params, delay_Q=delay_Q, policy_state_mask=policy_state_mask,
                         ...
                         n_fits=n_fits, logging_iter=logging_iter, adapt_tau_on_expert=adapt_tau_on_expert, lossQ_type=lossQ_type,
    ...
    for i in trange(1, n_epochs+1, 1, leave=False):
        with catchtime() as t:
            ...
            J, R, L, dataset = do_evaluation_deterministic(mdp, core, n_eval_episodes, tb_writer, i, return_dataset=True)
