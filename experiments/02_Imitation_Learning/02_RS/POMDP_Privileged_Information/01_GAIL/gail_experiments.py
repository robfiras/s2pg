...
from s2pg.networks.recurrent_networks import PPOActorRecurrentNetwork
from s2pg.policy.recurrent_policy import StatefulPolicyPPO_joint_prev_a_preprocess
from s2pg.utils.training import do_evaluation, do_evaluation_deterministic, BestAgentSaver
from s2pg.algorithms import GAIL_RS

from experiment_launcher import run_experiment


def experiment(horizon: int = 1000,
               gamma: float = 0.99,
               env: str = "HumanoidTorque",
               n_epochs: int = 300,
    ...
    policy = StatefulPolicyPPO_joint_prev_a_preprocess(network=PPOActorRecurrentNetwork,
                                                       input_shape=(np.sum(env_mask_policy), ),
                                                       output_shape=(dim_action+hidden_state_dim,),
                                                       n_features=128,
                                                       rnn_type=rnn_type,
    ...
    # setup critic
    input_shape_critic = (mdp.info.observation_space.shape[0]+n_hidden_features,)
    ...
    agent = GAIL_RS(mdp_info=mdp_info, policy=policy, sw=sw,
