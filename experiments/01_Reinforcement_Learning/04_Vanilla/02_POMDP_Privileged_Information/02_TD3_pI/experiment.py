...
from s2pg.networks.vanilla_networks import FullyConnectedNetwork,  TD3ActorNetwork
...
from s2pg.algorithms import TD3
...
def experiment(
    ...
    sigma = np.diag(np.ones(mdp.info.action_space.shape[0]) * std**2)
    policy_params = dict(sigma=sigma,
                         low=mdp.info.action_space.low,     # todo: Warning, humanoid env has low, high != -1,1 -> should used scaled tanh!
    ...
    # Create the agent
    agent = TD3(mdp_info=mdp.info,
                policy_params=policy_params,
                actor_params=actor_params, sw=sw, logging_iter=1000,
